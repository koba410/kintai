...
@section('content')
    <div class="container">
        <h2 class="text-left mt-5 mb-5">| 勤怠詳細</h2>

        <form method="POST" action="{{ route('attendance.correction', $attendance->id) }}">
            @csrf
            @method('POST')
                    ...
                    <input type="time" class="form-control @error('check_in') is-invalid @enderror" name="check_in"
                        value="{{ old(
                            ...
                            $hasPendingCorrection
                                ? \Carbon\Carbon::parse($hasPendingCorrection->corrected_check_in)->format('H:i')
                                : ($attendance->check_in
                ...
                <div class="col-sm-5">
                    <input type="time" class="form-control @error('check_out') is-invalid @enderror" name="check_out"
                        value="{{ old(
                            'check_out',
                            $hasPendingCorrection
                                ? \Carbon\Carbon::parse($hasPendingCorrection->corrected_check_out)->format('H:i')
                            ...
                            <div class="col-sm-5">
                                <input type="time"
                                    ...
                                    value="{{ old(
                                        ...
                                        isset($hasPendingCorrection->breakCorrections[$index])
                                            ? \Carbon\Carbon::parse($hasPendingCorrection->breakCorrections[$index]->corrected_start_time)->format('H:i')
                            ...
                            <div class="col-sm-5">
                                <input type="time"
                                    ...
                                    value="{{ old(
                                        ...
                                        isset($hasPendingCorrection->breakCorrections[$index])
                                            ? \Carbon\Carbon::parse($hasPendingCorrection->breakCorrections[$index]->corrected_end_time)->format('H:i')
            ...
            <div class="form-group row mt-3">
                ...
                <div class="col-sm-10">
                    <textarea class="form-control @error('note') is-invalid @enderror" id="note" name="note" rows="3"
                        {{ $hasPendingCorrection ? 'disabled' : '' }}>{{ old('note', optional($hasPendingCorrection)->reason ?? $attendance->note) }}</textarea>
                    @error('note')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
